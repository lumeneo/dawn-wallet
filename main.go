...
var (
	...
	qmlBridge                   *QmlBridge
	transfers                   []lumeneowalletdrpcgo.Transfer
	...
	stringBackupKeys            = ""
	rateUSDTRTL                 float64 // USD value for 1 LMO
...
func main() {
	...
	if isPlatformDarwin {
		...
		pathToAppFolder := pathToHomeDir + "/Library/Application Support/dawn-wallet"
	...
	log.WithField("version", versionDawn).Info("Application started")
	...
	go func() {
		newVersionAvailable, urlNewVersion = checkIfNewReleaseAvailableOnGithub(versionDawn)
	...
	walletdmanager.GracefullyQuitLumeneod()
}
...
func connectQMLToGOFunctions() {
	...
	qmlBridge.ConnectClickedButtonImport(func(filenameWallet string, passwordWallet string, privateViewKey string, privateSpendKey string, mnemonicSeed string, confirmPasswordWallet string, scanHeight string) {
		go func() {
			importWalletWithWalletInfo(filenameWallet, passwordWallet, confirmPasswordWallet, privateViewKey, privateSpendKey, mnemonicSeed, scan Height)
		}()
	})
	...
	if err == nil {
		...
		if needFullUpdate {
			...
			for index, transfer := range transfers {
				...
				amountString += " LMO (fee: " + strconv.FormatFloat(transfer.Fee, 'f', 2, 64) + ")"
...
func transfer(transferAddress string, transferAmount string, transferPaymentID string, transferFee string, transferMixin string) {
	...
	qmlBridge.FinishedSendingTransaction()
	qmlBridge.DisplayPopup("LMOs sent successfully", 4000)
}
