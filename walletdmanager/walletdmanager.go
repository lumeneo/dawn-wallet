...
import (
	"dawn-wallet/lumeneowalletdrpcgo"
	"bufio"
	"io"
	"math/rand"
	"os"
...
func RequestBalance() (availableBalance float64, lockedBalance float64, totalBalance float64, err error) {
	...
	availableBalance, lockedBalance, totalBalance, err = lumeneowalletdrpcgo.RequestBalance(rpcPassword)
	if err != nil {
...
func RequestAddress() (address string, err error) {
	...
	address, err = lumeneowalletdrpcgo.RequestAddress(rpcPassword)
	if err != nil {
...
func RequestListTransactions() (transfers []lumeneowalletdrpcgo.Transfer, err error) {

	walletBlockCount, _, _, err := lumeneowalletdrpcgo.RequestStatus(rpcPassword)
	if err != nil {
		log.Error("error getting block count: ", err)
		return nil, err
	}

	transfers, err = lumeneowalletdrpcgo.RequestListTransactions(walletBlockCount, 1, []string{WalletAddress}, rpcPassword)
	if err != nil {
...
func SendTransaction(transferAddress string, transferAmountString string, transferPaymentID string, transferFeeString string, transferMixinString string) (transactionHash string, err error) {
	...
	if !strings.HasPrefix(transferAddress, "dark") || (len(transferAddress) != 98 && len(transferAddress) != 187) {
	...
	if transferAmount <= 0 {
		return "", errors.New("amount of LMO to be sent should be greater than 0")
	}
	...
	if err != nil {
 	...
 	transactionHash, err = lumeneowalletdrpcgo.SendTransaction(transferAddress, transferAmount, transferPaymentID, transferFee, int(transferMixin), rpcPassword)
	if err != nil {
...
func OptimizeWalletWithFusion() (transactionHash string, err error) {
	...
	transactionHash, err = lumeneowalletdrpcgo.SendFusionTransaction(smallestOptimizedThreshold, DefaultTransferMixin, []string{WalletAddress}, WalletAddress, rpcPassword)
	if err != nil {
...
func getOptimisedFusionParameters() (largestFusionReadyCount int, smallestOptimizedThreshold int, err error) {
	...
	for {
		fusionReadyCount, _, err := lumeneowalletdrpcgo.EstimateFusion(threshold, []string{WalletAddress}, rpcPassword)
		if err != nil {
...
func GetPrivateKeys() (isDeterministicWallet bool, mnemonicSeed string, privateViewKey string, privateSpendKey string, err error) {
	...
	isDeterministicWallet, mnemonicSeed, err = lumeneowalletdrpcgo.GetMnemonicSeed(WalletAddress, rpcPassword)
	if err != nil {
		log.Error("error requesting mnemonic seed. err: ", err)
		return false, "", "", "", err
	}

	privateViewKey, err = lumeneowalletdrpcgo.GetViewKey(rpcPassword)
	if err != nil {
		log.Error("error requesting view key. err: ", err)
		return false, "", "", "", err
	}

	privateSpendKey, _, err = lumeneowalletdrpcgo.GetSpendKeys(WalletAddress, rpcPassword)
	if err != nil {
...
func SaveWallet() (err error) {
	...
	err = lumeneowalletdrpcgo.SaveWallet(rpcPassword)
	if err != nil {
...
func StartWalletd(walletPath string, walletPassword string, useRemoteNode bool, daemonAddress string, daemonPort string) (err error) {
	...
	pathToLogLumeneodCurrentSession := logLumeneodCurrentSessionFilename
	pathToLogLumeneodAllSessions := logLumeneodAllSessionsFilename
	pathToWalletd := "./" + walletdCommandName
	pathToLumeneod := "./" + lumeneodCommandName
	...
	if isPlatformDarwin {
		...
		pathToLumeneod = pathToAppContents + "/" + lumeneodCommandName
		...
		pathToAppLibDir := pathToHomeDir + "/Library/Application Support/dawn-wallet"
		...
		pathToLogLumeneodCurrentSession = pathToAppLibDir + "/" + logLumeneodCurrentSessionFilename
		pathToLogLumeneodAllSessions = pathToAppLibDir + "/" + logLumeneodAllSessionsFilename
		...
		pathToLumeneod = pathToAppDirectory + "/" + lumeneodCommandName
		...
		pathToLogLumeneodCurrentSession = pathToAppDirectory + "/" + logLumeneodCurrentSessionFilename
		pathToLogLumeneodAllSessions = pathToAppDirectory + "/" + logLumeneodAllSessionsFilename
		if pathToWallet == WalletFilename {
			// if comes from createWallet, so it is not a full path, just a filename
	...
	var LumeneodCurrentSessionLogFile *os.File
	...
	if !useRemoteNode && !isLumeneodRunning() {

		LumeneodCurrentSessionLogFile, err = os.Create(pathToLogLumeneodCurrentSession)
		if err != nil {
			log.Error(err)
		}
		defer LumeneodCurrentSessionLogFile.Close()

		cmdLumeneod = exec.Command(pathToLumeneod, "--log-file", pathToLogLumeneodCurrentSession)
		hideCmdWindowIfNeeded(cmdLumeneod)

		LumeneodAllSessionsLogFile, err := os.Create(pathToLogLumeneodAllSessions)
		if err != nil {
			log.Error(err)
		}
		cmdLumeneod.Stdout = LumeneodAllSessionsLogFile
		defer LumeneodAllSessionsLogFile.Close()

		err = cmdLumeneod.Start()
		if err != nil {
			log.Error(err)
			return err
		}

		log.Info("Opening Lumeneod and waiting for it to be ready.")

		readerLumeneodLog := bufio.NewReader(LumeneodCurrentSessionLogFile)

		for {
			line, err := readerLumeneodLog.ReadString('\n')
			if err != nil {
				if err != io.EOF {
					log.Error("Failed reading Lumeneod log file line by line: ", err)
				}
			}
			if strings.Contains(line, "Imported block with index") {
				log.Info("Lumeneod importing blocks: ", line)
			}
			if strings.Contains(line, "Core rpc server started ok") {
				log.Info("Lumeneod ready (rpc server started ok).")
				break
			}
			if strings.Contains(line, "Node stopped.") {
				errorMessage := "Error Lumeneod: 'Node stopped'"
	...
	_, _, _, err = lumeneowalletdrpcgo.RequestStatus(rpcPassword)
	if err != nil {
		killWalletd()
...
func GracefullyQuitWalletd() {
	...
	if WalletdOpenAndRunning || cmdWalletd != nil {
		...
		if isPlatformWindows {
			...
			lumeneowalletdrpcgo.SaveWallet(rpcPassword)
...
func GracefullyQuitLumeneod() {
	...
	if cmdLumeneod != nil {
		...
		if isPlatformWindows {
			...
			err = cmdLumeneod.Process.Kill()
			if err != nil {
				log.Error("failed to kill Lumeneod: " + err.Error())
			} else {
				log.Info("Lumeneod killed without error")
			}
		} else {
			_ = cmdLumeneod.Process.Signal(syscall.SIGTERM)
			done := make(chan error, 1)
			go func() {
				done <- cmdLumeneod.Wait()
			}()
			select {
			case <-time.After(5 * time.Second):
				if err := cmdLumeneod.Process.Kill(); err != nil {
					log.Warning("failed to kill Lumeneod: " + err.Error())
				}
				log.Info("Lumeneod killed as stopping process timed out")
			case err := <-done:
				if err != nil {
					log.Warning("Lumeneod finished with error: " + err.Error())
				}
				log.Info("Lumeneod killed without error")
			}
		}
...
func RequestConnectionInfo() (syncing string, blockCount int, knownBlockCount int, peerCount int, err error) {
	...
	blockCount, knownBlockCount, peerCount, err = lumeneowalletdrpcgo.RequestStatus(rpcPassword)
	if err != nil {
		return "", 0, 0, 0, err
	}
...
func isLumeneodRunning() bool {

	if _, _, err := findProcess(lumeneodCommandName); err == nil {
		return true
	}

	if isPlatformWindows {
		if _, _, err := findProcess(lumeneodCommandName + ".exe"); err == nil {
			return true
		}
	}
